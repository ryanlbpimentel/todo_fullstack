...
export async function updateTask(title, description, completed, id, user_id) {
    const result = await pool.query("UPDATE tasks SET title = COALESCE($1, title), description = COALESCE($2, description), completed = COALESCE($3, completed) WHERE id = $4 AND user_id = $5 RETURNING id", [title !== undefined ? title.trim() : null, description !== undefined ? description : null, completed !== undefined ? completed : null, id, user_id]);
    return result.rows[0];
}

export async function deleteTask(id, user_id) {
    const result = await pool.query("DELETE FROM tasks WHERE id = $1 AND user_id = $2 RETURNING id", [id,user_id]);
    return result.rows[0];
}